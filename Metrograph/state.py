...
All data lives in the `metrograph_showtimes` table (created by
migrations/002_metrograph.sql). Each row represents one showtime the bot
...
def log_discoveries(new_showtimes: list[dict]) -> None:
    ...
    sql = f"""
        INSERT INTO {_TABLE}
            (id, title, show_date, show_time, show_datetime,
             ...
             film_url, ticket_url, first_seen, hour, weekday)
    ...
    try:
        ...
        logger.debug("Logged %d Metrograph discovery row(s).", len(rows))
    except Exception as exc:
        logger.error("Failed to log Metrograph discoveries to DB: %s", exc)
